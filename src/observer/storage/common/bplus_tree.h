...
public:
    IndexFileHeader *get_index_header(){
      return &file_header_;
    }
    /**
     ...
     */
    RC insert_entry(const char *pkey, const RID *rid, int unique);
...
protected:
    ...
    RC check_unique(PageNum leaf_page, const char *pkey, const RID *rid);
